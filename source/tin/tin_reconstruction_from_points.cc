...
// See https://doc.cgal.org/latest/Manual/tuto_reconstruction.html
int main(int argc, char* argv[]) {
  ...
  // Do not use template parameter. See https://github.com/CGAL/cgal/issues/4748
  ...
  if (reconstruction_choice == 0 && false)  // Poisson
  {
    // If you want to use this mode, plz update CGAL version from 5.0.2
    // to 5.1.1.
    CGAL::jet_estimate_normals<CGAL::Sequential_tag>(points,
                                                     24);  // Use 24 neighbors
  ...
  } else if (reconstruction_choice == 1)  // Advancing front
  {
    ...
    CGAL::Polygon_mesh_processing::polygon_soup_to_polygon_mesh(
        vertices, facets, output_mesh);
    std::ofstream f("reconstructed_mesh.off");
    f << output_mesh;
    f.close();
  } else if (reconstruction_choice == 2)  // Scale space
  {
    ...
    reconstruct.increase_scale(
        4, CGAL::Scale_space_reconstruction_3::Jet_smoother<Kernel>());
    ...
    std::ofstream f("out.off");
    f << "OFF" << std::endl
      << points.size() << " " << reconstruct.number_of_facets() << " 0"
      << std::endl;
  ...
  {
    std::cerr << "Error: invalid reconstruction id: " << reconstruction_choice
              << std::endl;
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
